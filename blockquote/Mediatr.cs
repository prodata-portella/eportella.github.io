...
internal sealed class SvgFormatterRequest : IRequest<string?>
{
    public string? Content { get; init; }
}
internal sealed class SvgFormatterRequestHandler : IRequestHandler<SvgFormatterRequest, string?>
{
    public async Task<string?> Handle(SvgFormatterRequest request, CancellationToken cancellationToken)
    {
        return SvgFormatter.Format(request.Content);
    }
...
internal sealed class BuildRequestHandler(IMediator mediator) : IRequestHandler<BuildRequest>
{
    public async Task Handle(BuildRequest request, CancellationToken cancellationToken)
    {
        ...
        var content = await mediator.Send(new BlockquoteFormatterRequest { FileInfo = request.FileInfo }, cancellationToken);
        content = await mediator.Send(new SvgFormatterRequest { Content = content }, cancellationToken);
